#====================================================================================#
# Use for Epistemic Uncertainity Analysis
# Will be using a deterministic model sampling from distributions for model parameters
# Starting from v2Sensativity_Analysis, but implementing changes based on reviewwer comments
# Created by EK on 3/7/2023
#====================================================================================#
...
#MODEL CONSTANTS
#Hog constants
beta_Hog <- EnvStats::rtri(n = 1, min = 0.0001, max = 0.02, mode = (10/(1000*5)))
...
#Human constants
#applied the beta = Ro / N*D from the modeling book
beta_WF2WF <-   EnvStats::rtri(n = 1, min = 0.0000001, max = 0.64, mode = 0.32) # http://m-hikari.com/ams/ams-2013/ams-41-44-2013/joseAMS41-44-2013.pdf 
Hum_Vacc_Eff <- 0.0 
...
TransmissionModel.par <- c(beta_Hog = beta_Hog, beta_Hog_ind = beta_Hog_ind, beta_S2WF = beta_S2WF, beta_WF2S = beta_WF2S,
                           ...
                           qrate = qrate, beta_qHog_ind = beta_qHog_ind, q_cap = q_cap, beta_qS2WF = beta_qS2WF,
...
TransmissionModel.dyn <- function(t,var,par) {
  ...
  # dS1 <- (w_Hog*R1) - (beta_Hog*I1*S1) - (beta_Hog_ind*(0)*S1) - (beta_WF2S*HI*S1) - (u_Hog*S1)
  # dE1 <-(beta_Hog*I1*S1) + (beta_Hog_ind*(0)*S1) + (beta_WF2S*HI*S1) - sigma_Hog*E1 - (u_Hog*E1)
  dI1 <- (sigma_Hog*E1) - (delta_Hog*I1) - (qrate*I1) - (u_inf_Hog*I1)
  dQ1 <- (qrate*I1) - (delta_Hog*Q1) - (u_inf_Hog*Q1) 
  ...
  dD1_Inf <- (u_inf_Hog*I1) + (u_inf_Hog*Q1)
  ...
  # dS2 <- (w_Hog*R2) - (beta_Hog*I2*S2) - (beta_Hog_ind*(0)*S2) - (beta_WF2S*HI*S2) - (u_Hog*S2)
  # dE2 <-(beta_Hog*I2*S2) + (beta_Hog_ind*(0)*S2) + (beta_WF2S*HI*S2) - sigma_Hog*E2 - (u_Hog*E2)
  dI2 <- (sigma_Hog*E2) - (delta_Hog*I2) - (qrate*I2) - (u_inf_Hog*I2)
  dQ2 <- (qrate*I2) - (delta_Hog*Q2) - (u_inf_Hog*Q2) 
  ...
  dD2_Inf <- (u_inf_Hog*I2) + (u_inf_Hog*Q2)
  ...
  # dS3 <- (w_Hog*R3) - (beta_Hog*I3*S3) - (beta_Hog_ind*(0)*S3) - (beta_WF3S*HI*S3) - (u_Hog*S3)
  # dE3 <-(beta_Hog*I3*S3) + (beta_Hog_ind*(0)*S3) + (beta_WF3S*HI*S3) - sigma_Hog*E3 - (u_Hog*E3)
  dI3 <- (sigma_Hog*E3) - (delta_Hog*I3) - (qrate*I3) - (u_inf_Hog*I3)
  dQ3 <- (qrate*I3) - (delta_Hog*Q3) - (u_inf_Hog*Q3) 
  ...
  dD3_Inf <- (u_inf_Hog*I3) + (u_inf_Hog*Q3)
  ...
  dI4 <- (sigma_Hog*E4) - (delta_Hog*I4) - (qrate*I4) - (u_inf_Hog*I4)
  dQ4 <- (qrate*I4) - (delta_Hog*Q4) - (u_inf_Hog*Q4) 
  ...
  dD4_Inf <- (u_inf_Hog*I4) + (u_inf_Hog*Q4)
  ...
  # dHS <- (w_WF*HR)- (beta_WF2WF*HI*HS) - (beta_S2WF*(I1)*HS) - (u_WF*HS)
  # dHE <- (beta_WF2WF*HI*HS) + (beta_S2WF*(I1)*HS) - (sigma_WF*HE) - (u_WF*HE)
  dHI <- (sigma_WF*HE) - (delta_WF*HI) - (u_inf_WF*HI)
  dHR <- (delta_WF*HI) - (w_WF*HR) - u_WF*HR
  dH_DN <- (u_WF*HS) + (u_WF*HE) + (u_WF*HR)
  dH_DI <- (u_inf_WF*HI)
  dHog2WF_Count <- (beta_S2WF*(I1)*HS)
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/EpiUncertianity Analysis/March 7 Updates/5Qrate_Sum_EpiUAnlaysis.rds")
...
for (i in c(1:n.itr)) {
  ...
  TransmissionModel.dyn <- function(t,var,par) {
    ...
    #Room 2
    dS2 <- (w_Hog*R2) - (beta_Hog*I2*S2) - (beta_Hog_ind*(I3+I4)*S2) - (beta_qHog_ind*(Q1+Q2+Q3+Q4)*S2) - (beta_WF2S*HI*S2) - (u_Hog*S2)
    dE2 <-(beta_Hog*I2*S2) + (beta_Hog_ind*(I3+I4)*S2) + (beta_qHog_ind*(Q1+Q2+Q3+Q4)*S2) + (beta_WF2S*HI*S2) - sigma_Hog*E2 - (u_Hog*E2)
    # dS2 <- (w_Hog*R2) - (beta_Hog*I2*S2) - (beta_Hog_ind*(0)*S2) - (beta_WF2S*HI*S2) - (u_Hog*S2)
    # dE2 <-(beta_Hog*I2*S2) + (beta_Hog_ind*(0)*S2) + (beta_WF2S*HI*S2) - sigma_Hog*E2 - (u_Hog*E2)
    dI2 <- (sigma_Hog*E2) - (delta_Hog*I2) - (qrate*I2) - (u_inf_Hog*I2)
    dQ2 <- (qrate*I2) - (delta_Hog*Q2) - (u_inf_Hog*Q2) 
    ...
    #Room3
    dS3 <- (w_Hog*R3) - (beta_Hog*I3*S3) - (beta_Hog_ind*(I4)*S3) - (beta_qHog_ind*(Q1+Q2+Q3+Q4)*S3) - (beta_WF2S*HI*S3) - (u_Hog*S3)
    dE3 <-(beta_Hog*I3*S3) + (beta_Hog_ind*(I4)*S3) + (beta_qHog_ind*(Q1+Q2+Q3+Q4)*S3) + (beta_WF2S*HI*S3) - sigma_Hog*E3 - (u_Hog*E3)
    # dS3 <- (w_Hog*R3) - (beta_Hog*I3*S3) - (beta_Hog_ind*(0)*S3) - (beta_WF3S*HI*S3) - (u_Hog*S3)
    # dE3 <-(beta_Hog*I3*S3) + (beta_Hog_ind*(0)*S3) + (beta_WF3S*HI*S3) - sigma_Hog*E3 - (u_Hog*E3)
    dI3 <- (sigma_Hog*E3) - (delta_Hog*I3) - (qrate*I3) - (u_inf_Hog*I3)
    dQ3 <- (qrate*I3) - (delta_Hog*Q3) - (u_inf_Hog*Q3) 
    ...
    #Room 4
    dS4 <- (w_Hog*R4) - (beta_Hog*I4*S4) - (beta_Hog_ind*(0)*S4) - (beta_qHog_ind*(Q1+Q2+Q3+Q4)*S4) - (beta_WF2S*HI*S4) - (u_Hog*S4)
    dE4 <-(beta_Hog*I4*S4) + (beta_Hog_ind*(0)*S4) + (beta_qHog_ind*(Q1+Q2+Q3+Q4)*S4) + (beta_WF2S*HI*S4) - sigma_Hog*E4 - (u_Hog*E4)
    # dS4 <- (w_Hog*R4) - (beta_Hog*I4*S4) - (beta_Hog_ind*(0)*S4) - (beta_WF2S*HI*S4) - (u_Hog*S4)
    # dE4 <-(beta_Hog*I4*S4) + (beta_Hog_ind*(0)*S4) + (beta_WF2S*HI*S4) - sigma_Hog*E4 - (u_Hog*E4)
    dI4 <- (sigma_Hog*E4) - (delta_Hog*I4) - (qrate*I4) - (u_inf_Hog*I4)
    dQ4 <- (qrate*I4) - (delta_Hog*Q4) - (u_inf_Hog*Q4) 
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/EpiUncertianity Analysis/March 7 Updates/Room4_Sum_EpiUAnlaysis.rds")
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/EpiUncertianity Analysis/March 7 Updates/Room3_Sum_EpiUAnlaysis.rds")
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/EpiUncertianity Analysis/March 7 Updates/Room2_Sum_EpiUAnlaysis.rds")
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/EpiUncertianity Analysis/March 7 Updates/Room1_Sum_EpiUAnlaysis.rds")
