...
# Created by EK on 2/14/2023 
# builds from Vaccination r scripts

# Using this file to test effects of improved PPE workers and sick worker introduced on day 42
...
beta_S2WF <- (0.6 / (4002*5)) - (0.91*(0.6 / (4002*5)))  #When workers wear N95
...
# beta_WF2S <- (0.83 / (4002*3)) - (0.68*(0.83 / (4002*3)))    #for wearing mask
beta_WF2S <- (0.83 / (4002*3)) - (0.91*(0.83 / (4002*3)))    #for wearing N95 mask

#Human constants
#applied the beta = Ro / N*D from the modeling book
# beta_WF2WF <- (1.92 / (2*3)) # http://m-hikari.com/ams/ams-2013/ams-41-44-2013/joseAMS41-44-2013.pdf 
# beta_WF2WF <- (1.92 / (2*3)) - (0.6*(1.92 / (2*3)))    #for wearing cloth mask
beta_WF2WF <- (1.92 / (2*3)) - (0.91*(1.92 / (2*3)))    #for wearing N95 mask
Hum_Vacc_Eff <- 0.0 
...
TransmissionModel_Room1MDA.ODE <- c(
  #Room 1
  #Susceptible
  "w_Hog*R1", "(beta_Hog_MDA*I1*S1)", "(beta_Hog_ind_MDA*(I2+I3+I4)*S1)", "(beta_WF2S*HI*S1)", "u_Hog*S1",
  #Exposed
  ...
  #Room 2
  #Susceptible
  "w_Hog*R2", "(beta_Hog*I2*S2)", "(beta_Hog_ind*(I1+I3+I4)*S2)", "(beta_WF2S*HI*S2)", "u_Hog*S2",
  #Exposed
  ...
  #Room 3
  #Susceptible
  "w_Hog*R3", "(beta_Hog*I3*S3)", "(beta_Hog_ind*(I1+I2+I4)*S3)", "(beta_WF2S*HI*S3)", "u_Hog*S3",
  #Exposed
  ...
  #Room 4
  #Susceptible
  "w_Hog*R4", "(beta_Hog*I4*S4)", "(beta_Hog_ind*(I1+I2+I3)*S4)", "(beta_WF2S*HI*S4)", "u_Hog*S4",
  #Exposed
...
TransmissionModel_Room2MDA.ODE <- c(
  ...
  #Room 2
  #Susceptible
  "w_Hog*R2", "(beta_Hog_MDA*I2*S2)", "(beta_Hog_ind_MDA*(I1+I3+I4)*S2)", "(beta_WF2S*HI*S2)", "u_Hog*S2",
  #Exposed
  "sigma_Hog*E2", "u_Hog*E2",
  #Infected
  "(delta_Hog*I2)", "u_inf_Hog*I2",
...
TransmissionModel_Room3MDA.ODE <- c(
  ...
  #Room 3
  #Susceptible
  "w_Hog*R3", "(beta_Hog_MDA*I3*S3)", "(beta_Hog_ind_MDA*(I1+I2+I4)*S3)", "(beta_WF2S*HI*S3)", "u_Hog*S3",
  #Exposed
  "sigma_Hog*E3", "u_Hog*E3",
  #Infected
  "(delta_Hog*I3)", "u_inf_Hog*I3",
...
TransmissionModel_Room4MDA.ODE <- c(
  ...
  #Room 4
  #Susceptible
  "w_Hog*R4", "(beta_Hog_MDA*I4*S4)", "(beta_Hog_ind_MDA*(I1+I2+I3)*S4)", "(beta_WF2S*HI*S4)", "u_Hog*S4",
  #Exposed
  "sigma_Hog*E4", "u_Hog*E4",
  #Infected
  "(delta_Hog*I4)", "u_inf_Hog*I4",
...
for (i in c(1:n.itr)) {
  ...
  TransmissionModel.Int.6to7 <- c(
    ...
    #Room4
    S4 = round((1-Hog_Vacc_Eff )*1000, digits = 0),
    E4 = 0,
    I4 = 0,
    R4 = round(Hog_Vacc_Eff *1000, digits = 0),
    Total_inf4 = 0,
    True_Rec4 = 0,
    D4_Nat = 0,
    D4_Inf = 0,
    ...
    #Workforce
    HS = round((1-Hum_Vacc_Eff )*1, digits = 0), 
    HE = 0, 
    HI = 1, 
    HR = round(Hum_Vacc_Eff *1, digits = 0), 
    H_DN = 0, 
    H_DI = 0,
    Hog2WF_Count = 0
  )
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/Improved PPE/R10_N95PPE.rds")
...
#   mutate(TimeSineInfIntro = t - 42) %>%
#   ungroup() %>%
#   summarise(mean_time = mean(TimeSineInfIntro),
#             #mean_realtime = mean(TimeSineInfIntro),
#             #med_time = median(t),
#             med_realtime = median(TimeSineInfIntro),
#             range_realtime = range(TimeSineInfIntro),
#             LB_realtime = quantile(TimeSineInfIntro, probs = .05),
#             UB_realtime = quantile(TimeSineInfIntro, probs = .95))
# 
# ## Time to Second Room Infection
