...
# Created by EK on 2/4/2023 
# builds from WF Flow r scripts

# Using this file to test effects of Vaccination only
...
TransmissionModel_Room2MDA.ODE <- c(
  ...
  #Room 2
  #Susceptible
  "w_Hog*R2", "(beta_Hog_MDA*I2*S2)", "(beta_Hog_ind_MDA*(I3+I4)*S2)", "(beta_WF2S*HI*S2)", "u_Hog*S2",
  #Exposed
  "sigma_Hog*E2", "u_Hog*E2",
  #Infected
  "(delta_Hog*I2)", "u_inf_Hog*I2",
...
TransmissionModel_Room3MDA.ODE <- c(
  ...
  #Room 3
  #Susceptible
  "w_Hog*R3", "(beta_Hog_MDA*I3*S3)", "(beta_Hog_ind_MDA*(I4)*S3)", "(beta_WF2S*HI*S3)", "u_Hog*S3",
  #Exposed
  "sigma_Hog*E3", "u_Hog*E3",
  #Infected
  "(delta_Hog*I3)", "u_inf_Hog*I3",
...
TransmissionModel_Room4MDA.ODE <- c(
  ...
  #Room 4
  #Susceptible
  "w_Hog*R4", "(beta_Hog_MDA*I4*S4)", "(beta_Hog_ind_MDA*(0)*S4)", "(beta_WF2S*HI*S4)", "u_Hog*S4",
  #Exposed
  "sigma_Hog*E4", "u_Hog*E4",
  #Infected
  "(delta_Hog*I4)", "u_inf_Hog*I4",
...
for (i in c(1:n.itr)) {
  ...
  TransmissionModel.Int.0to2 <- c(
    #Room1
    S1 = round((1-Hog_Vacc_Eff )*999, digits = 0),
    E1 = 0,
    I1 = 1,
    R1 = round(Hog_Vacc_Eff *999, digits = 0),
    Total_inf1 = 0,
    True_Rec1 = 0,
    D1_Nat = 0,
    D1_Inf = 0,
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/WF Flow/Indirect by 178/R10_Room1_60VaccEff.rds")
...
results %>% 
  group_by(iter) %>% 
  summarise(t_cross = t[which(E2 >= 1 | 
                                E3 >= 1 |
                                E4 >= 1)]) %>% 
  summarise(min_t_cross = min(t_cross)) %>% 
  ungroup() %>% 
  summarise(med_t_cross = median(min_t_cross),
            LB_t_cross = quantile(min_t_cross, probs = 0.05),
            UB_t_cross = quantile(min_t_cross, probs = 0.95))
...
results %>% 
  ...
  ungroup() %>% 
  summarise(med_t_peak = median(min_t_peak),
            LB_t_peak = quantile(min_t_peak, probs = 0.05),
            UB_t_peak = quantile(min_t_peak, probs = 0.95))
