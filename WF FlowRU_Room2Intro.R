...
for (i in c(1:n.itr)) {
  ...
  TransmissionModel.Int.2to3 <- c(
    ...
    #Room2
    S2 = round((1-Hog_Vacc_Eff )*999, digits = 0),
    E2 = 0,
    I2 = 1,
    R2 = round(Hog_Vacc_Eff *999, digits = 0),
    Total_inf2 = 0,
    True_Rec2 = 0,
    D2_Nat = 0,
    D2_Inf = 0,
...
saveRDS(results, file = "Current code/Reviewer Update Code and Models/WF Flow/Indirect by 178/R10_Room2_60VaccEff.rds")
...
results %>% 
  ...
  ungroup() %>% 
  summarise(med_t_cross = median(min_t_cross) - 14,
            LB_t_cross = quantile(min_t_cross, probs = 0.05) - 14,
            UB_t_cross = quantile(min_t_cross, probs = 0.95) - 14)
...
results %>% 
  ...
  ungroup() %>% 
  summarise(med_t_peak = median(min_t_peak) - 14,
            LB_t_peak = quantile(min_t_peak, probs = 0.05) - 14,
            UB_t_peak = quantile(min_t_peak, probs = 0.95) - 14)
