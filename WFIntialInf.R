...
# Using this file to WF Inf introduction and different mask wearing
#================================================================#
#### LIBRARIES ####
# library(tidyverse)
# library(GillespieSSA)

#### PAPERS FOR IMPROVED PPE ####
#Physical Interventions to interrupt or reduce the spread of respiraotyr viruses: Systematic review (Table 6)
#https://www.bmj.com/content/bmj/336/7635/77.full.pdf?casa_token=Z9kTomM80MoAAAAA:Az9oade3d6Id9Y4I_X305gMgJIRrzy_djrCs4raHiXWZRZh0MkjIqmFzYfOznZr9amJkC_-EpaY
#### MODEL CONSTANTS ####
#Hog constants
# beta_Hog <- 0.285   #high transmision
# beta_Hog <- (10/(1000*5))   #Ro 10 transmision  10/(4000*5)
...
# beta_WF2WF <- (1.92 / (2*3)) - (0.99*(1.92 / (2*3)))    #for wearing N95 mask
...
for (i in c(1:n.itr)) {
  ...
  TransmissionModel.Int.6to8 <- c(
    ...
    #Workforce
    HS = round((1-Hum_Vacc_Eff )*1, digits = 0), #need to add a +1 for 5 workers b/c round function drops one 
    HE = 0, 
    HI = 1, 
    HR = round(Hum_Vacc_Eff *1, digits = 0), # +1 for 5 workers to either susceptible or recovered
    H_DN = 0, 
    H_DI = 0,
    Hog2WF_Count = 0
    # #Workforce
...
saveRDS(results, file = "Current code/Control Meausres/Improved PPE/Indirect equals Direct by 178/LT_80HogVaccEff_N95PPE.rds")
# results <- readRDS()In
### Reporting Values ####
